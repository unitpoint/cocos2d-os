...
void initOpenGL(MarmaladeOS * os)
{
	struct OpenGL
	{
...
void initOpenGL2(MarmaladeOS * os)
{
	struct OpenGL
	{
...
void initOpenGLExt(MarmaladeOS * os)
{
	...
	struct OpenGL
	{
		...
		{
			float color[4];
			if(!((MarmaladeOS*)os)->getColor(-params, color)){
				for(int i = 0; i < 4; i++){
					color[i] = !os->isNumber(-params+i) ? 1.0f : MarmaladeOS::clampUnit(os->toFloat(-params+i));
				}
			}
